  ...
  class UpdateCalendarEventTool < BaseTool
    ...
    # Find the event (try both our ID and Google event ID)
      ...
      when "cancel"
        ...
        error_response("Invalid action: #{action}. Must be 'update', 'reschedule', or 'cancel'")
      end
    rescue => e
      error_response("Error updating calendar event: #{e.message}")
    end
  ...
  def find_event(event_id)
    ...
    begin
      ...
      updated_google_event = google_service.update_event('primary', event.google_event_id, google_event)
        ...
        error_response("Google Calendar API error: #{e.message}")
      end
    end

    def reschedule_event(event, params)
      unless params["new_start_time"] && params["new_end_time"]
        return error_response("Both new_start_time and new_end_time are required for rescheduling")
      end
    ...
    def cancel_event(event, params)
      ...
      begin
        ...
        success_response(
          "Calendar event cancelled successfully",
          {
            event: {
              id: event.id,
              google_event_id: event.google_event_id,
